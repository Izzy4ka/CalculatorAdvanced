...
class ExpressionParserImpl : ExpressionParser {
    ...
    override fun parse(tokens: List<Token>): ExpressionNode {
        return InnerParser(tokens).parseRoot()
    }

    private class InnerParser(private val tokens: List<Token>) {
        private var position: Int = 0

        fun parseRoot(): ExpressionNode {
        ...
        private fun parseUnary(): ExpressionNode {
            val operator =
                match<Token.Operator.Minus>() ?: match<Token.Operator.Plus>()
                ?: return parsePrimary()
